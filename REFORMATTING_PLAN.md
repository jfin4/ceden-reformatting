...
5. **Rule 5: Strict row dropping criterion.**
   - A row may be dropped **if and only if** it is completely unparseable:
     $$\text{Drop Row} \iff \text{is.na}(StationCode) \land \text{is.na}(AnalyteName) \land \text{is.na}(Result)$$
...
1. **Extraction Target Directory:**
   Extract each archive `refs/ref<ID>.zip` into a dedicated subdirectory:
   $$\text{Extraction Path: } ./refs\_extracted/ref\langle ID\rangle/$$
   Example: `refs/ref4448.zip` extracts to `./refs_extracted/ref4448/`.

2. **Recursive Archive Handling:**
   Certain archives contain nested zip files (e.g., `ref2518.zip` contains `Benthic.zip`, `Chemistry.zip`, `ToxData.zip`, `TrawlData.zip`).
...
3. **Artifact Sanitization:**
   During or immediately following extraction, prune operating system artifacts and corrupt non-data files:
...
4. **Standalone vs. Extracted Archive Precedence:**
   In approximately 147 instances, a reference ID exists as both a standalone spreadsheet (e.g., `refs/ref4448.xlsx`) and an archive (e.g., `refs/ref4448.zip` containing `ShastaTailwaterReduction_WaterChem_Sediment_Field.xlsx`).
...
5. **File Provenance Tracking:**
   Every record processed from an extracted file must retain its exact origin:
   - `RefID`: e.g., `"ref4448"`
...
1. **Scan Sheet Names:**
   Retrieve all sheet names via `readxl::excel_sheets()`.
2. **Identify Metadata Sheets:**
   Do **not** process sheets named `ReadMe`, `Readme`, `README`, `Title`, `title`, `Table of contents`, `Qualifier Definitions`, `StationCode List`, `Delisting Criteria`, `Notes`, `Study References`, `BPExceedance Tables`, or `Graphs` as result tables.
...
2. For each row $i \in \{1, \dots, 25\}$:
   - Convert row values to character strings, trim whitespace, and discard empty strings.
   - Compute a **Header Match Score** ($S_i$) based on the count of matching CEDEN keyword tokens:
     $$S_i = \sum \mathbb{I}(\text{token} \in \{\text{"station"}, \text{"sample"}, \text{"date"}, \text{"time"}, \text{"analyte"}, \text{"parameter"}, \text{"result"}, \text{"matrix"}, \text{"method"}, \text{"unit"}, \text{"qualifier"}, \text{"latitude"}, \text{"longitude"}\})$$
3. **Decision Rule:**
   - The header row is the row index $k$ that maximizes $S_k$, provided $S_k \ge 3$.
   - If the first row already has $S_1 \ge 3$, set header row $k = 1$.
   - If $k > 1$, re-read the sheet setting `skip = k - 1` and `col_names = TRUE`.
   - Any metadata in rows $1$ to $k-1$ (such as station coordinates embedded in a title block) must be parsed into an auxiliary metadata dictionary.

### 4.4 Row Filtering & Summary Row Dropping Specification

Once the tabular data is read into memory:

1. **Evaluate Rule 5 Criterion:**
   For every row $r$, evaluate whether:
   $$\text{is.na}(r.\text{StationCode}) \land \text{is.na}(r.\text{AnalyteName}) \land \text{is.na}(r.\text{Result})$$
2. **Execution Action:**
   - If **TRUE**: Drop row $r$. Increment `rows_dropped_unparseable` counter.
   - If **FALSE**: Retain row $r$.
3. **Accounting:**
   Record in `_metadata.csv`:
...
| **7**  | **`MatrixName`**           | Character (CV)         | Sample matrix (e.g., `samplewater`, `sediment`).        | Crosswalk source matrix (e.g., `"water"` $\to$ `"samplewater"`). If missing, infer from sheet name or analyte type.         |
...
| **10** | **`FractionName`**         | Character (CV)         | Chemical fraction (e.g., `Total`, `Dissolved`, `None`). | Crosswalk source fraction (`Filtered` $\to$ `Dissolved`). Default to `None` for field physical parameters (pH, Temp).       |
...
- **Base Export URL:**
  `https://ceden.org/CEDEN_Checker/Checker/CEDEN_export_xls.php?table=<TableName>`
- **Output Format:**
  Even though the script is named `CEDEN_export_xls.php`, the server returns standard Office OpenXML `.xlsx` files (`Content-Type: application/octet-stream`, PK zip header).
- **Caching Logic:**
...
`StationLookUp` contains tens of thousands of California stations (~2.2 MB compressed download). If the HTTP request times out due to server load:
1. Re-attempt download using `curl` with retry parameters: `curl -L --retry 3 --connect-timeout 30 -o ./cv_lookups/StationLookUp.xlsx "<URL>"`. Convert to `./cv_lookups/StationLookUp.csv`.
2. If the CEDEN server remains unreachable, proceed using source-provided coordinates only. Set `station_lookup_status = "offline_fallback"` in the metadata CSV sidecars and flag that station lookup verification was deferred.
...
Before evaluating archetype-specific mappings, cleanse all source column names:
1. Strip leading and trailing whitespace.
2. Remove non-printable / control characters.
3. Apply the following known typo corrections:
...
- `CollectionMethodName` or `CollectionMethodCode` $\to$ `CollectionMethodCode`
- `Replicate` or `CollectionReplicate` $\to$ `Replicate`
- `MatrixName` $\to$ `MatrixName`
- `MethodName` $\to$ `MethodName`
- `AnalyteName` (or `Analyte`) $\to$ `AnalyteName`
- `FractionName` $\to$ `FractionName`
- `UnitName` (or `Unit`) $\to$ `UnitName`
- `Result` $\to$ `Result`
- `ResQualCode` $\to$ `ResQualCode`
- `QACode` $\to$ `QACode`
- `MDL` $\to$ `MDL`
- `RL` $\to$ `RL`
- `Latitude` / `TargetLatitude` / `SampleLatitude` / `SampleLatitiude` $\to$ `TargetLatitude`
- `Longitude` / `TargetLongitude` / `SampleLongitude` $\to$ `TargetLongitude`
- `Datum` / `SampleDatum` $\to$ `Datum`
- `SampleTypeCode` $\to$ `SampleTypeCode`
- `CollectionDepth` $\to$ `CollectionDepth`
- `UnitCollectionDepth` $\to$ `UnitCollectionDepth`
- `LabBatch` $\to$ `LabBatch`
- `LabSampleID` $\to$ `LabSampleID`
- `AnalysisDate` $\to$ `AnalysisDate`
- `LabReplicate` $\to$ `LabReplicate`
- `SampleComments` $\to$ `SampleComments`
- `LabResultComments` $\to$ `LabResultComments`

#### Special Handling for Tissue Sheets:
- Map `CommonName` or `FinalID` $\to$ `CommonName`
- Map `TissueName` $\to$ `TissueName`
- Map `TissuePrep` $\to$ `TissuePrep`

#### Special Handling for Habitat Sheets:
- If `Result` is empty but `VariableResult` is populated, copy `VariableResult` $\to$ `Result` if numeric; if categorical, copy to `ResQualCode` or `Observation` and record in comments.

---

### 7.3 Archetype 2: CIWQS Regulatory Exports (`CIWQS`)

CIWQS exports (e.g., `ref5180.xlsx`) map identically to Archetype 1:
- `WQID` $\to$ record in `LabResultComments` as `"CIWQS_WQID: <val>"`
- Columns `StationCode` through `ResQualCode` map 1-to-1 with Section 7.2.

---

### 7.4 Archetype 3: WQX / Water Quality Portal Exports (`WQX`, `WaterQualityPortal`)

WQX spreadsheets (e.g., `ref5831.xlsx`) use standard National WQX naming:
- `MonitoringLocationIdentifier` or `StationCode` $\to$ `StationCode`
- `ActivityStartDate` or `SampleDate` $\to$ `SampleDate`
- `ActivityStartTime/Time` or `CollectionTime` $\to$ `CollectionTime`
- `ProjectIdentifier` or `ProjectCode` $\to$ `ProjectCode`
- `ActivityMediaName` or `MatrixName` $\to$ `MatrixName`
- `CharacteristicName` or `AnalyteName` $\to$ `AnalyteName`
- `ResultSampleFractionText` or `FractionName` $\to$ `FractionName`
- `ResultMeasureValue` or `Result` $\to$ `Result`
- `ResultMeasure/MeasureUnitCode` or `UnitName` $\to$ `UnitName`
- `ResultDetectionConditionText` $\to$ Crosswalk to `ResQualCode` (e.g., `"Not Detected"` $\to$ `"ND"`)
- `DetectionQuantitationLimitMeasure/MeasureValue` $\to$ `MDL` or `RL`

---

### 7.5 Archetype 4: CCAMP / Legacy Regional Board Formats (`Ccamp_WQ_Data`, `Swamp_WQ_Data`)

Files such as `ref2334.xls` and `ref2335.xls` use CCAMP/SWAMP flat layouts:
- `StationCode` $\to$ `StationCode`
- `SampleDate` $\to$ `SampleDate`
- `SampleTime` $\to$ `CollectionTime`
- `ProjectID` $\to$ `ProjectCode`
- `SampleReplicate` $\to$ `Replicate`
- `MatrixName` $\to$ `MatrixName`
- `MethodName` $\to$ `MethodName`
- `AnalyteName` $\to$ `AnalyteName`
- `FractionName` $\to$ `FractionName`
- `Unit` $\to$ `UnitName`
- `Result` $\to$ `Result`
- `ResultQualCode` $\to$ `ResQualCode`
- `QACode` $\to$ `QACode`
- `MDL` $\to$ `MDL`
- `RL` $\to$ `RL`
- `DepthSampleCollection` $\to$ `CollectionDepth`
- `DepthUnit` $\to$ `UnitCollectionDepth`
- `SampleTypeCode` $\to$ `SampleTypeCode`
- `AgencyCode` $\to$ `SampleAgencyCode`
- `Latitude` / `Longitude`: If values equal `"Not Recorded"`, convert to `NA`.

---

### 7.6 Archetype 5: CDPR Pesticide Database (`CDPR_Data`, `ref2966.txt`)

Pesticide monitoring tables from the California Department of Pesticide Regulation (DPR):
- `site description` $\to$ `StationName`
- `StationCode`: Leave blank (`""`) per Rule 2, unless a station lookup dictionary is provided in the study.
- `date` $\to$ `SampleDate`
- `chemical` $\to$ `AnalyteName`
- `Conc [ug/L]` $\to$ `Result` (numeric)
- `loq` (Limit of Quantitation) $\to$ `RL`
- `study_cd` $\to$ `ProjectCode` (e.g., `"CDPR_STUDY_<study_cd>"`)
- `latitude` $\to$ `TargetLatitude`
- `longitude` $\to$ `TargetLongitude`
- **Mandatory Default Mappings for CDPR:**
  - `UnitName`: `"ug/L"` (explicitly declared in header `Conc [ug/L]`)
  - `MatrixName`: `"samplewater"` (ambient surface water pesticide runoff)
  - `FractionName`: `"Total"`
  - `CollectionMethodCode`: `"Water_Grab"`
  - `MethodName`: `"EPA 680"` or `"Unspecified"` (flag in metadata CSV)
  - `ResQualCode`: If `Result == 0` or `Result < loq`, set `ResQualCode = "ND"`, else `"="`.
...
2. **Column Mappings within Sheets:**
   ...
   - `Result` $\to$ `Result`
   - `Unit` $\to$ `UnitName`
   - `ResQualifier` $\to$ `ResQualCode`
   - `MDL` $\to$ `MDL`
   - `RL` $\to$ `RL`
   - `QACodeDescr` $\to$ `QACode`
   - `MatrixName` $\to$ `MatrixName`
   - `MethodName` $\to$ `MethodName`
   - `ActualLatitude` $\to$ `TargetLatitude`
   - `ActualLongitude` $\to$ `TargetLongitude`
   - `Datum` $\to$ `Datum`
   - `ProjectID` $\to$ `ProjectCode`
3. **Union / Combine:**
   Append records across all analyte sheets for the reference ID into a unified dataset.
...
Columns represent individual metals: `AG_W`, `AS_W`, `CD_W`, `CR_W`, `CU_W`, `HG_W`, `NI_W`, `PB_W`, `SE_W`, `ZN_W`.
- Base columns:
  - `STANUM` $\to$ `StationCode`
  - `STANAME` $\to$ `StationName`
  - `CDATE` $\to$ `SampleDate`
  - `COMMON` $\to$ `CommonName`
  - `TISSUE` $\to$ `TissueName` (crosswalk: `"FF"` $\to$ `"Fish Fillet"`, `"WF"` $\to$ `"Whole Fish"`)
- **Reshape Algorithm:**
  Pivot wide columns (`*_W`) longer into key-value pairs:
  - `AnalyteCode` = Column name (e.g., `"HG_W"`) $\to$ Crosswalk to CEDEN name:
    - `AG_W` $\to$ `"Silver"`
    - `AS_W` $\to$ `"Arsenic"`
    - `CD_W` $\to$ `"Cadmium"`
    - `CR_W` $\to$ `"Chromium"`
  ...
  - `MatrixName` = `"tissue"`
  - `FractionName` = `"Total"`
  - `UnitName` = `"mg/Kg ww"` (wet weight) for `MET_WET`; `"mg/Kg dw"` (dry weight) for `ORG_DRY`.

#### Pattern B: USGS Station-Per-Sheet (`ref2449.xls`)
Each sheet is named after a USGS station (e.g., `USGS9427520`).
- Extract `StationCode` from the sheet name: e.g., `"10259540"` (strip `"USGS"` prefix if alphanumeric).
- Columns represent USGS Parameter Codes (P-Codes) or constituent names.
- Map row dates $\to$ `SampleDate`.
- Pivot parameter columns to long format: `AnalyteName` and `Result`.
...
- **Extraction Rules:**
  ...
  - `SampleDate`: Parse `20080723` $\to$ `"2008-07-23"`
  - `CollectionTime`: Parse `0445` $\to$ `"04:45:00"`
  - `Result`: `1.50`
  - `ResQualCode`: `"="`
  - `QACode`: `"None"`
...
1. **Excel Numeric Date Serial:**
   - If the raw value is numeric (e.g., $30,000 \le X \le 60,000$):
     $$\text{SampleDate} = \text{as.Date}(X, \text{origin} = \text{"1899-12-30"})$$
2. **Text / Character Date:**
   - Clean string: remove trailing timestamp components if date-only is desired.
   - Parse using `lubridate::parse_date_time()` with formats:
     $$\text{orders} = c(\text{"Ymd"}, \text{"mdY"}, \text{"dmY"}, \text{"Ymd HMS"}, \text{"mdY HMS"})$$
...
1. **Excel DateTime / Time Serial:**
   ...
   - If raw time is an Excel fraction of a day ($0 \le X < 1$):
     $$\text{Seconds} = \text{round}(X \times 86400)$$
     Format as `HH:MM:SS`.
2. **Text String Parsing:**
   - Format `"HH:MM"` $\to$ Append `":00"`.
   - Format `"H:MM:SS AM/PM"` $\to$ Convert to 24-hour `HH:MM:SS`.
   - Format `"HHMM"` (e.g., `"0445"`) $\to$ Format as `"04:45:00"`.
...
| :---                                                                        | :---                        | :---                                              |
| `Filtered`, `filter`, `Dissolved`, `DIS`, `Diss`, `D`, `Soluble`            | `Dissolved`                 | Material passing filter ($\le 0.45\,\mu\text{m}$) |
...
1. **Detection Limit Extraction Criteria:**
   An MDL/RL extracted from a QAPP may be applied **only if** the QAPP table explicitly matches both the `AnalyteName` and `MethodName`.
2. **Ambiguity Rule:**
   If the QAPP lists multiple project phases with different detection limits, or if the method does not match, **do not guess**. Assign `NA` and document the ambiguity in `_metadata.csv`.
...
1. **Coordinate Verification Cascade:**
   - Step 1: Check source data columns (`TargetLatitude`, `TargetLongitude`). If valid decimal numbers (Latitude $32.0 \le \text{Lat} \le 42.5$; Longitude $-125.0 \le \text{Long} \le -114.0$), retain source coordinates. Set `coordinates_from_source` counter.
...
1. A row is dropped if and only if:
   $$\text{is.na}(\text{StationCode}) \land \text{is.na}(\text{AnalyteName}) \land \text{is.na}(\text{Result})$$
...
1. **Reformatted Data CSV (`.csv`):**
   ...
   - File naming template:
     $$\text{output/ref}\langle\text{ID}\rangle\_\langle\text{sheet\_or\_table\_name}\rangle\text{.csv}$$
     Example: `output/ref4708_WaterSedimentField.csv`
     If sub-files exist in archives: `output/ref4448_ShastaTailwater_WaterChem.csv`
   - Specifications:
...
2. **Metadata Sidecar CSV (`_metadata.csv`):**
   ...
   - File naming template:
     $$\text{output/ref}\langle\text{ID}\rangle\_\langle\text{sheet\_or\_table\_name}\rangle\_metadata\text{.csv}$$
     Example: `output/ref4708_WaterSedimentField_metadata.csv`
   - Schema structure: A standardized 4-column Entity-Attribute-Value (EAV) table:
     $$\text{Columns: } \text{Section}, \text{Property}, \text{Value}, \text{Notes}$$
...
2. **Row Count Conservation:**
   - Read the output CSV row count ($N_{\text{out}}$).
   - Assert:
     $$\text{raw\_rows\_read} == N_{\text{out}} + \text{rows\_dropped\_unparseable}$$
   - If false, halt with an accounting error.
3. **Required Columns Check:**
   Verify that all 14 required columns exist as column headers in `output/<dataset_id>.csv`:
   $$\{\text{"StationCode"}, \text{"SampleDate"}, \dots, \text{"QACode"}\} \subseteq \text{names}(\text{read\_csv}(\dots, \text{n\_max} = 1))$$
...
5. **Coordinate Bounding Box Check:**
   For all non-empty coordinates in the output CSV:
   $$32.0 \le \text{as.numeric}(TargetLatitude) \le 42.5 \quad \land \quad -125.0 \le \text{as.numeric}(TargetLongitude) \le -114.0$$
   Any coordinates falling outside this boundary must be logged as a flag in the metadata CSV.
...
| :---                                                         | :---                                                        | :---                                                                                                                                                              |
| **All column names are `...1`, `...2`, `...3`**              | Table has leading title rows or metadata block above header | Execute Section 4.3 Header Detection Algorithm. Find row where CEDEN keyword count $\ge 3$. Skip preceding rows.                                                  |
